...
#define LOG_VRB(fmt, ...) LOG(5, fmt, ## __VA_ARGS__)
...
#	define LOG_HEXDUMP(level, data, len, _start, _end) \
 	   do { \
		if ((level) <= (LOG_LEVEL)) { \
			_log_printf(_start); \
...
#	define LOG_HEXDUMP_WRN(data, len) LOG_HEXDUMP(2, data, len, LOG_COLOR_WRN, LOG_COLOR_RESET "\n")
#	define LOG_HEXDUMP_ERR(data, len) LOG_HEXDUMP(1, data, len, LOG_COLOR_ERR, LOG_COLOR_RESET "\n")
